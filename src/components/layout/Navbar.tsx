...
import { Search, Bell, Film, Sparkles, ChevronDown, Check, Compass, Bookmark, Tv } from 'lucide-react';
      ...
      <div className="max-w-[1720px] mx-auto px-4 sm:px-8 flex items-center justify-between">
        <div className="flex items-center space-x-8 lg:space-x-10">
          ...
          <nav className="hidden md:flex items-center space-x-6 text-sm font-medium">
            {[{id:'home',label:'Home'},{id:'series',label:'Series'},{id:'movies',label:'Movies'},{id:'universes',label:'My Universes'},{id:'watchlist',label:'My List'}].map((link) => (
        ...
        <div className="flex items-center space-x-3 sm:space-x-5">
          <button onClick={onOpenCreateStudio} className="flex items-center space-x-2 px-3.5 py-1.5 rounded-full bg-gradient-to-r from-purple-600/90 to-pink-600/90 text-white text-xs font-semibold shadow-lg border border-white/10 hover:scale-105 transition-all"><Sparkles className="w-3.5 h-3.5" /><span className="hidden sm:inline">Director Studio</span><span className="sm:hidden">Create</span></button>
          ...
          <div className="relative">
            <button onClick={() => { setNotificationsOpen(!notificationsOpen); setProfileDropdownOpen(false); }} className="relative p-2 text-gray-300 hover:text-white rounded-full hover:bg-white/5"><Bell className="w-4 h-4" /><span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full bg-pink-500" /></button>
          ...
          <div className="relative">
            <button onClick={() => { setProfileDropdownOpen(!profileDropdownOpen); setNotificationsOpen(false); }} className="flex items-center space-x-2 p-1 pl-1.5 pr-2.5 rounded-full bg-white/5 hover:bg-white/10 border border-white/10">
